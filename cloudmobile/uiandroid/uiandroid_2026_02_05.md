...
Для борьбы с этим есть модификатор `sealed`. `sealed` ограничивает наследование на этапе компиляции, то есть все подклассы должны быть объявлены в том же файле, а также запрещает наследование от данного класса только на этапе исполнения, когда как на этапе компиляции наследование разрешено
