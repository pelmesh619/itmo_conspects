...
+ Алгоритм Брезенхэма
    ...
    Отсюда $S = (d y)/(d x) (r+1) - q$

    Так как $S + T = 1$, $T = 1 - S$. Умножая разность $S - T$ на $d x$, получаем $d x (S - T) = 2(r+1) d y - (2q+1) d x$
    ...
    - Если $d_i < 0$, выбираем пиксель $S_i$с координатами $(x_i, y_i)$, где $y_i = y_(i-1)$, поэтому $d_(i+1) = d_i + 2d y$
