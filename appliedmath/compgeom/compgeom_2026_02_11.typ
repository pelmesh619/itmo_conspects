...
- Индикатор ориентации
    ...
    #Def Псевдоскалярное (или косое) произведение $arrow(a) or arrow(b) #equaldef |arrow(a)| |arrow(b)| sin(arrow(a), arrow(b))$, причем произведение положительно, если угол между $arrow(a)$ и $arrow(b)$ положителен (то есть против часовой), и отрицательно, если угол отрицателен (то есть по часовой)
...
Вектор $vec(m, n)$ -- вектор переноса $T_((m, n))$, а число $s$ отвечает за общее масштабирование 
