...
Пусть первый игрок имеет $n$ вариантов игры -- $A_1, A_2, dots, A_n$, а второй имеет варианты $B_1, B_2, dots, B_m$

То есть при одной игры первый игрок выбирает один из $n$ вариантов, а второй из других $m$ вариантов
...
Второе следствие: алгоритм решения игры $2 times 2$. Пусть матрица $A = mat(a_(1 1), a_(1 2); a_(2 1), a_(2 2))$, тогда:
...
+ Если не имеет, то обе стратегии должны быть активными, и должно выполняться неравенство. Пусть $P^* = (p_1, p_2)$ и $Q^* = (q_1, q_2)$ -- оптимальные стратегии, тогда получаем две системы из трех уравнений:
  ...
  $ cases(p_1 + p_2 = 1, a_(1 1) p_1 + a_(2 1) p_2 = nu, a_(1 2) p_1 + a_(2 2) p_2 = nu) #h(2cm) cases(q_1 + q_2 = 1, a_(1 1) q_1 + a_(1 2) q_2 = nu, a_(2 1) q_1 + a_(2 2) q_2 = nu) $

Рассмотрим этот алгоритм для орлянки: $A = mat(-1, 1; 1, -1)$
