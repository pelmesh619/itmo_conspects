...
\begin{document}
    ...
    \begin{itemize}
        \item дискретным -- $\xi \in \{0, 1, 2, \dots\}$

        \item непрерывным -- $\xi \in [0; \infty)$
    \end{itemize}
    ...
    \Def Случайная величина $\xi$ имеет дискретное распределение, если она принимает не более, чем счетное число значений.
    ...
    \begin{enumerate}
        \item $m_k = E\xi^k$ -- момент $k$-ого порядка случайной величины $\xi$ (также называют начальным моментом)

        $E\xi = m_1$ -- момент первого порядка

        $E\xi^2 = m_2$ -- момент второго порядка

        \item $\mu_k = E(\xi - E\xi)^k$ -- центральный момент $k$-ого порядка

        $\mu_2 = D\xi = E(\xi - E\xi)^2$ -- центральный момент второго порядка
    \end{enumerate}
