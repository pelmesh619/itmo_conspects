...
\begin{document}
    ...
    Док-во: из определения, если $B = \Real$

    3) $F_\xi(x) = \int_B f_\xi(x)dx$

    Док-во, если $B = (-\infty; x)$, то $F_\xi(x) = p(\xi \in (-\infty; x)) = \int_{-\infty}^x f_\xi(x)dx$

    4) $F_\xi(x)$ непрерывна (из свойства непрерывности интеграла с верхним переменным пределом)

    5) $F_\xi(x)$ дифференцируема почти везде и $f_\xi(x) = F^\prime_\xi(x)$ для почти всех $x$ (по теореме Барроу)
