...
\begin{document}
    ...
    \begin{enumerate}
        \item Если $\xi \in N(a, \sigma^2)$, то $\eta = \gamma \xi + b \in N(a\gamma + b; \gamma^2 \sigma^2)$
        ...
        \item Если $\eta \in N(0, 1)$ -- стандартное нормальное распределение, то $\xi = \sigma \eta + a \in N(a, \sigma^2)$

        \item Если $\eta \in U(0, 1)$ -- стандартное равномерное распределение и $a > 0$, то $\xi = a\eta + b \in U(b, a + b)$

        \item Если $\xi \in E_\alpha$, то $\alpha \xi \in E_1$
    \end{enumerate}
    ...
    \begin{MyProof}
        ...
        \begin{enumerate}
            \item $F(x)$ -- строго возрастающая функция. Тогда $\exists F^{-1}(x)$ -- обратная, $F_\eta(x) = p(\eta < x) = p(F(\xi) < x) = 
            \begin{cases}0, & x < 0 \\ p(\xi < F^{-1}(x)) = F(F^{-1}(x)) = x, & 0 \leq x \leq 1 \text{ -- функция распределения } U(0, 1) \\ 1, & x > 1 \end{cases}$

            \item $F(x)$ -- не является строго возрастающей функцией -- то есть существуют участки постоянства, в этом случае
