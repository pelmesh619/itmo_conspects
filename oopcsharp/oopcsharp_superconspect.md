...
Также сделаем декоратор `StyledExpressionDecorator` - он реализует интерфейс `IExpression` и содержит в себе модификатор для текста, при вызове метода `Format` он будет возвращать сформатированный текст. Аналогично сделаем для интерфейса `IExpressionValue`
...
**Model** содержит бизнес-логику, требует изменений, когда изменяются случаи использования и бизнес-правила системы
...
Луковая (также "чистая", "clean") архитектура основывается на гексагональной, но в отличии от нее компонент с бизнес-логикой разделяется на части с доменными сервисами (Domain Services) и доменными моделями (Domain Model)

* Доменные модели - определения бизнес-правил
* Доменные сервисы - определения случаев использования
