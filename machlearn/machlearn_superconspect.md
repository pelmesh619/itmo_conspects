...
* Среднее значение абсолютных разностей (Mean Average Error) - $\mathrm{MAE} = \frac{1}{n} \sum_{i = 1}^{n} |y_i - y_i^\prime|$
...
4. Вычисляем расстояния от точки $A$ до точек $x_i$. Это можно делать с помощью:
    ...
    * Метрики Манхэттена $\mathrm{dist}(x, y) = \sum_{i} |x_i - y_i|$ - лучше устойчива к выбросам в пространствах высокой размерности
...
* Равномерная функция (Uniform Kernel) $$K(x) = \begin{cases} \frac{1}{2}, & \|x\| \leq 1 \\ 0, & \|x\| \geq 1\end{cases}$$ - то есть все веса равны 1
* Треугольная функция (Triangle Kernel) $$K(x) = \begin{cases}1 - \|x\|, & \|x\| \leq 1 \\ 0, & \|x\| \geq 1\end{cases}$$
* Епанечникова (параболическая) функция $$K(x) = \begin{cases}\frac{3}{4} (1 - x^2), & \|x\| \leq 1 \\ 0, & \|x\| \geq 1\end{cases}$$
