...
2. К этому произведению добавляется смещение $b_i$ и взвешенное предыдущего результата $w_i y_{(t - 1)}_i$: $W_i x^{(t)} + b_i + w_i y_{(t - 1)}_i$
3. Применяется функция активации, например, ReLU: $y_{(t)}_i = \mathrm{ReLU}(W_i x^{(t)} + b_i + w_i y_{(t - 1)}_i)$
4. Значение $y^{(t)}_i$ используется в следующем слое и для вычислении следующего значения этого нейрона

![Нейрон в рекуррентной нейросети]()
...
1. Вентиль забывания (forget gate): $c^\prime_t = c_{t - i} \cdot \sigma(h_{t - 1} w x_t + b)$

    Значение $\sigma(h_{t - 1} w x_t + b)$ определяет, какой процент состояния ячейки нужно запомнить

2. Входной вентиль (input gate): $c_t = c^\prime_t + \sigma(р_{t - 1} \omega x_t + \beta) \cdot \mathrm{tanh}(р_{t - 1} \psi x_t + \gamma)$

    Здесь решается, какую долю входного значения и скрытого состояния нужно запомнить

3. Выходной вентиль (output gate): $h_t = \mathrm{tanh}(c_t) \cdot \sigma(h_{t - 1} v x_t + \lambda)$

    Значение $h_t$ передается на следующий слой нейронов или как вывод

Здесь $w$, $\omega$, $\psi$ и $v$ - вектора весов, а $b$, $\beta$, $\gamma$ и $\lambda$ - смещения. Через их обучения сеть начинает понимать, какие данные запоминать, а какие забывать
