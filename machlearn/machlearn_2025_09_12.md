...
3. Robust-масштабирование
    ...
    $$x_{\text{норм}} = \frac{x - \operatorname{median}(x)}{\operatorname{IQR}(x)}$$, где $$\operatorname{median}(x)$$ - медиана, $$\operatorname{IQR}(x)$$ - разница между 25-ым и 75-ым квантилем


    Также формулу можно представить так: $$x_{\text{норм}} = \frac{x - \operatorname{Q_2}(x)}{\operatorname{Q_3}(x) - \operatorname{Q_1}(x)}$$, где $$\operatorname{Q_1}(x), \operatorname{Q_2}(x), \operatorname{Q_3}(x)$$ - квантили выборки уровней $$0.25$$, $$0.5$$, $$0.75$$ соответственно
...
* Критерий знаков используется для проверки гипотезы о медиане распределения выборки
    ...
    При нулевой гипотезе $$H_0 \ : \ m = m_0$$, вероятность того, что наблюдение окажется выше или ниже медианы, равна $1/2$. Следовательно, статистика $S$ имеет биномиальное распределение: $$S \in B(n, p = \frac{1}{2}).$$
    ...
    Для больших $$n$$ биномиальное распределение аппроксимируется нормальным: $$S \rightrightarrows N\left(\frac{n}{2}, \frac{n}{4}\right)$$
    ...
    Для больших $$n$$ можно использовать нормальную аппроксимацию: $$K = \frac{S - n/2}{\sqrt{n}/2} \rightrightarrows N(0,1)$$
...
* Критерий Манна–Уитни (или U-критерий) применяется для проверки гипотезы о равенстве распределений двух независимых выборок
    ...
    Для больших выборок берем статистику $$Z = \frac{U - nm/2}{\sqrt{nm(n+m+1)/12}} \rightrightarrows N(0,1)$$
...
* Так как связь может быть не строго линейной, существует коэффициент корреляции Спирмана
    ...
    $$\rho = 1 - \frac{6 \sum_i d^2}{n (n^2 - 1)}$$, где $$d_i$$ - разность рангов
