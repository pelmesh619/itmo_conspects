...
\subsection{Модифицированная лемма Фишера}

\vspace{-2.5em}
\hfill \TagYTwoEight

\hypertarget{modified_fishers_lemma}{}

\begin{MyTheorem}
    Пусть вектор $\vec X$ -- стандартный нормальный вектор, который рассматриваем как радиус-вектор, 
    соответствующий точке в пространстве $\Real^n$. $L \subset \Real^n$ -- минимальное подпространство размерности $k$, 
    $\mathrm{dim}(L) = k$. Тогда квадрат расстояния от $\vec X$ до $L$ имеет распределение \enquote{хи-квадрат} со степенями свободы $n - k$,
    и не зависит от проекции $\vec X$ на $L$.
\end{MyTheorem}

\begin{MyProof}
    Пусть $f_1, \dots , f_k$ -- ортонормированный базис $L$;
    $f_{k+1}, f_{k+2}, \dots , f_n$ -- его ортонормированное дополнение до базиса $\Real^n$.
    Пусть $C$ -- матрица перехода от базиса $\{f_i\}$ к естественному базису $\{e_i\}$, поэтому она ортогональная.
    Координаты вектора $\vec X$ в новом базисе $\vec Y = C \cdot \vec X$. Тогда по свойству 5 $\vec Y$ -- тоже стандартный  нормальный вектор.
    $\vec Y = \vec {Y'} + \vec {Y''}$, где $\vec {Y'}$ -- проекция $\vec X$ на подпространство $L$,
    где $\vec {Y'} = (Y_1, \dots , Y_k, 0, \dots , 0)$, $\vec {Y''} = (0, \dots , Y_{k+1}, \dots , Y_n)$ -- перпендикуляр к $L$.
    Тогда квадрат расстояния от точки $X$ до $L$: $d^2 = Y_{k+1}^2 + \dots + Y_n^2$ -- по определению \enquote{хи-квадрат} со степенями свободы $n - k$
    и не зависит от координат проекции $\vec X$ на $L$. 
\end{MyProof}
...
\begin{MyProof}
    \begin{enumerate}
        ...
        \sum_{i = 1}^n (z_i - \overline{z})^2$, где $z_i = \frac{X_i - a}{\sigma} \in N(0, 1)$, $\overline{z} = \frac{1}{n} \sum_{i = 1}^n z_i = \frac{\sum_{i = 1}^n X_i - na}{n \sigma} = \frac{\overline{x} - a}{\sigma}$
        ...
        \setcounter{enumi}{3}
        \item $\sqrt{n} \frac{\overline{x} - a}{S} = \sqrt{n} \frac{\overline{x} - a}{\sigma} \cdot \frac{1}{\sqrt{\frac{\frac{S^2 (n - 1)}{\sigma^2}}{n - 1}}} = \frac{\sqrt{n} \frac{\overline{x} - a}{\sigma}}{\sqrt{\frac{\chi^2_{n - 1}}{n - 1}}}$
...
\begin{MyProof}
    Альтернативное доказательство через модифицированную лемму Фишера \hfill \TagYTwoEight
    \begin{enumerate}
        \item Аналогично предыдущей версии

        \item Аналогично предыдущей версии
        
        \item $\sum_{i = 1}^n \frac{(X_i - \overline{x})^2}{\sigma^2} = \sum_{i = 1}^n \left(\frac{X_i - a}{\sigma} - \frac{\overline{x} - a}{\sigma}\right)^2 = 
        \sum_{i = 1}^n (Y_i - \overline{Y})^2$, где $Y_i = \frac{X_i - a}{\sigma} \in N(0, 1)$, $\overline{Y} = \frac{1}{n} \sum_{i = 1}^n Y_i = \frac{\sum_{i = 1}^n X_i - na}{n \sigma} = \frac{\overline{x} - a}{\sigma}$

        $\vec Y = (Y_1, \dots , Y_n)$ -- стандартный нормальный вектор.

        Тогда получаем: $\sum_{i = 1}^n \frac{(X_i - \overline{x})^2}{\sigma^2} = \sum_{i=1}^n (Y_i - \overline{Y})^2 = nD_Y^*$ -- квадрат расстояния от
        $\vec Y$ до прямой $l: x_1 = x_2 = \dots = x_n$, а $\overline{Y}$ -- проекция $\vec Y$ на эту прямую.

        Тогда по модифицированной лемме Фишера: $\sum_{i = 1}^n \frac{(X_i - \overline{x})^2}{\sigma^2} \in H_{n-1}$,
        
        \setcounter{enumi}{4}
        \item Согласно модифицированной лемме Фишера и выкладкам из пункта 3: 
        $\frac{(n-1)S^2}{\sigma}$ -- не зависит от $\overline{Y} = \frac{\overline{x} - a}{\sigma}$, следовательно $S^2$ не зависит от $\overline{x}$.
