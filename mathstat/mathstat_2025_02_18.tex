...
\begin{MyProof}
    ...
    2) $\overline{x} = \frac{x_1 + \dots + x_n}{n} \overset{p}{\longrightarrow} a$ 
...
Геометрическая интерпретация выборочного среднего и выборочной дисперсии   \hfill \TagYTwoEight

Пусть $a, \sigma^2$ - теоретические матожидание и дисперсия, а $\bar x$, $D^*$ - их оценки. \\
Выборку $\vec X = (X_1, \dots , X_n)$ будем рассматривать как точку в $n$-мерном пространстве \\
В $n$-мерном пространстве построим прямую, которая задается равенством $x_1 = x_2 = \dots = x_n$, где $x_i$ - координата по $i$-ой оси пространства \\
У этой прямой направляющий вектор назовем $\vec S = (1, 1, ..., 1)$ \\
Отметим на прямой две точки $A = (a, a, \dots , a)$ и $B = (\bar x, \bar x, \dots , \bar x)$

% https://www.geogebra.org/calculator/bxb8j8bm

\begin{center}
        \includegraphics[width=0.7\textwidth]{mathstat/images/mathstat_2025_02_18_1.png}
\end{center}
Так как $\overrightarrow{BX} \cdot \vec S = (x_1 - \bar x, x_2 - \bar x, \dots , x_n - \bar x) \cdot (1, 1, \dots , 1) = \sum_{i = 1}^{n}(x_i - \bar x) = \sum_{i = 1}^{n}x_i - \sum_{i = 1}^{n} \bar x = n \bar x - n \bar x = 0 \Rightarrow$
$\Rightarrow B = (\bar x, \bar x, \dots , \bar x)$ - проекция выборки на данную прямую \\
$|BX|^2 = \sum_{i = 1}^{n}(x_i - \bar x)^2 = n D^*$ \\
$|AX|^2 = \sum_{i = 1}^{n}(x_i - a)^2$ \\
$|AB|^2 = |AX|^2 - |BX|^2 = \sum_{i = 1}^{n}(x_i - a)^2 - n D^*$

Следствие: $\sum_{i = 1}^{n}(x_i - a)^2 = n(D^* + (\bar x - a)^2)$ \hfill \TagYTwoEight

\begin{MyProof}
    Из геометрического смысла выборочного среднего и выборочной дисперсии: \\
    $|AB|^2 = n (\bar x - a)^2$ \\
    Тогда доказываемое равенство очевидно
\end{MyProof}
