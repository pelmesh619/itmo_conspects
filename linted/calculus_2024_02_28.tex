...
\textit{Ex}:} Гамма-функция
    ...
    $\displaystyle \int^{+\infty}_1 x^{\alpha - 1} e^{-x} dx \leq \int^{+\infty}_1 x^{n} e^{-x} dx$ - по частям, появятся $\dislpaystyle x^{k} e^{-x} \Big|^{+\infty}_1 \rightarrow 0$ и $\displaystyle \dislpaystyle \int^{+\infty}_1 e^{-x} dx$ сходится
    ...
    \includegraphics[height=70mm]{images/calculus_28_02_1}

    \clearpage

    4. Функция нескольких переменных (ФНП)

    4.1. Определение

    \vspace{3mm}
\textit{Nota}: Дадим определение ФНП

    \includegraphics[height=90mm]{images/calculus_28_02_2}

    $\forall M(x, y) \exists! z \in \Real : z = f(x, y) \Longleftrightarrow z = f(x, y)$ - функция двух переменных

    \vspace{3mm}
\textit{Def}: Окрестность точки $M_0(x_0, y_0)$
    ...
    \vspace{3mm}
\textit{Nota}: $\Delta x = x - x_0, \Delta y = y - y_0$, одновременное стремление $\Delta x, \Delta y \rightarrow 0$
    можно заменить $\Delta \pho = \sqrt{(x - x_0)^2 + (y - y_0)^2} \rightarrow 0$\\[1\baselineskip]


    \vspace{3mm}
\textit{Def}: $\displaystyle \lim_{M \to M_0} z(x, y) = L \in \Real \Longleftrightarrow \forall \varepsilon > 0 \exists \delta > 0 (\delta = \delta(\varepsilon)) | \forall M \in \stackrel{o}{U}_\delta(M_0) \text{ } |z(x, y) - L| < \varepsilon$

    $M_0$ - точка сгущения и $x_0, y_0 \in \Real$ (здесь)

    \vspace{3mm}
\textit{Nota}: На плоскости $Oxy$ возможно стремление $M \rightarrow M_0$ по разным путям $F(x, y) = 0$ (уравнение кривой)
    ...
    \vspace{3mm}
\textit{Def}: $z = f(x, y)$ называется непрерывной в точке $M_(x_0, y_0)$, если $z = f(x_0, y_0) = \lim_{M \to M_0} z(x, y)$

    $z$ непрерывна на $D$, если $z$ непрерывна $\forall (x, y) \in D$

    \includegraphics[height=90mm]{images/calculus_28_02_3}

    \vspace{3mm}
\textit{Nota}: Справедливы теоремы Вейерштрасса и Больцано-Коши для функции, непрерывной в заданной области

    $z = f(x, y)$ непрерывна на $\overline{D} = D \union \Gamma_D$, где $\overline{D}$ - закрытая область, $D$ - открытая область, $\Gamma_D$ - граница

    \vspace{3mm}
\textit{Th}. W1 - $z = f(x, y)$ ограничена на $\overline{D}$

    \vspace{3mm}
\textit{Th}. W2 - $\exists$ наибольшее и наименьшее $z \in \overline{D}$

    \vspace{3mm}
\textit{Th}. B-C1 - на границе $\Gamma_D$ $z$ принимает значения разных знаков $\Longrightarrow \exists M \in \overline{D} : z(M) = 0$

    \vspace{3mm}
\textit{Th}. B-C1 - $z(x, y)$ принимает все значения от $z_{\text{наим}}$ до $z_{\text{наиб}}$ \\[1\baselineskip]
    ...
    \vspace{3mm}
\textit{Nota}: $\Delta_y z = z(x, y + \Delta y) - z(x, y)$ и  $\Delta_y z$ называют частным приращением

    \vspace{3mm}
\textit{Def}: Полное приращение $\Delta z \stackrel{def}{=} z(x + \Delta x, y + \Delta y) - z(x, y)$
