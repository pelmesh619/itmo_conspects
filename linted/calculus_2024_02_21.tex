...
\begin{document}
    ...
    \includegraphics[height=90mm]{images/calculus_21_02_1}
    ...
    \vspace{3mm}
\textit{Def}.1: Определим несобственный интеграл первого рода (на неограниченном промежутке) ($f(x)$ любого знака):

    \[\int^{+\infty}_{a} f(x) dx = \lim_{b \to +\infty} \int^{b}_{a} f(x) dx\]

    \vspace{3mm}
\textit{Nota}: Если этот предел существует и конечен, то говорят, что интеграл сходится. В противном случае расходится

    \vspace{3mm}
\textit{Def}.2: Функция определена на полуинтервале $[-\infty; b]$ и непрерывна. Тогда определен:

    \[\int^{b}_{-\infty} f(x) dx = \lim_{a \to -\infty} \int^{b}_{a} f(x) dx\]

    \vspace{3mm}
\textit{Def}.3: $\displaystyle \int^{+\infty}_{-\infty} f(x) dx = \int^{c}_{-\infty} f(x) dx + \int^{+\infty}_{c} f(x) dx$

    \vspace{3mm}
\textit{Nota}: Этот интеграл сходится, если сходятся оба интеграла справа, и расходится, если расходится хотя бы один из них
    (в том числе если возникает неопределенность $\infty - \infty$)

    \vspace{3mm}
\textit{Ex}: $\displaystyle f(x) = \frac{1}{x}$

    \includegraphics[height=90mm]{images/calculus_21_02_2}

    Сделаем ее непрерывной

    \includegraphics[height=90mm]{images/calculus_21_02_3}
    ...
    \textbf{
    \vspace{3mm}
\textit{Ex}.1:} \[\int^{+\infty}_{-\infty} \frac{dx}{1 + x^2} = arctg x \Big|^{+\infty}_{-\infty} = arctg x \Big|^{c = 0}_{-\infty} + arctg x \Big|^{+\infty}_{c = 0} =
    \lim_{x \to +\infty} arctgx - arctg(0) + arctg(0) - \lim_{x \to -\infty} arctgx = \frac{\pi}{2} + \frac{\pi}{2} = \pi\]

    \vspace{5mm}
    \textbf{
    \vspace{3mm}
\textit{Ex}.2:} \[\int^{+\infty}_{1} \frac{dx}{xlnx} = \int^{+\infty}_{1} \frac{dlnx}{lnx} = \int^{+\infty}_{0} \frac{dt}{t}
    ...
    \includegraphics[height=90mm]{images/calculus_21_02_4}

    $f(x) : [a; b) \to \Real$, где $b$ - точка разрыва второго рода, а именно бесконечного

    \vspace{3mm}
\textit{Def}.1: Интеграл второго рода (несобственный)
    ...
    \vspace{3mm}
\textit{Def}.2: Аналогично ($a$ - точка бесконечного разрыва):

    \[\int^{b}_{a} f(x) dx = \lim_{\alpha \to a} \int^{b}_{\alpha} f(x) dx\]

    \vspace{3mm}
\textit{Def}.3 $c \in [a;b]$ - точка бесконечного разрыва:
...
\textit{Ex}.2:}
    ...
    \vspace{3mm}
\textit{Nota}: Если нет разбиения $[a; b]$ по аддитивности, то неопределенности раскрываются

    \vspace{3mm}
\textit{Ex}: $\displaystyle \int^{2}_{1} \frac{dx}{x^2 - 1} = \frac{1}{2} \int^{2}_{1} (\frac{1}{x - 1} - \frac{1}{x + 1})dx =
    ...
    \vspace{3mm}
\textit{Nota}: Исследование интегралов двух функций используется для определения их сходимости
    ...
    \vspace{3mm}
\textit{Nota}: Можно аналогично сравнить функции отрицательного знака
    ...
    \vspace{3mm}
\textit{Nota}. Отметим, что если $f(x)$ не является убывающей к нулю, т. е. б. м. на $+\infty$, то $\displaystyle \int^{+\infty}_{a} f(x) dx$ разойдется
...
\textit{Nota}: Обратное неверно
    ...
    \vspace{3mm}
\textit{Nota}: Если $\displaystyle I = \int^{+\infty}_{a} f(x) dx$ сходится, но $\displaystyle |\int^{+\infty}_{a} f(x) dx|$ расходится, то $I$ называют условно сходящимся

    \vspace{3mm}
\textit{Ex}: $\displaystyle I = \int^{+\infty}_{a} \frac{sinx}{8x^2 + 3} dx$
