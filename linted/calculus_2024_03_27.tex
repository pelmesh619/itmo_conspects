...
\begin{document}
    \textbf{5. Интеграл ФНП}

    \vspace{5mm}

    \textbf{5.1 Общая схема интегрирования}
    ...
    \vspace{3mm}
\textit{Ex}. Область $\Omega$ - дуга кривой $l : y = y(x)$

    Скалярная функция $g(M)$ - плотность в точке $M$

    \vspace{3mm}
\textit{Ex}. Область $\Omega$ - трубка в $\Real^3$
    ...
    \vspace{3mm}
\textit{Ex}. Область $\Omega$ - кривая, по которой движется точка $M$ под действием силы $\overrightarrow{G}(M)$
    ...
    \vspace{3mm}
\textit{Ex}. Проиллюстрируем на примере $\displaystyle \int_a^b f(x) dx$
    ...
    \vspace{3mm}
\textit{Ex}. Сила $\overrightarrow{F}(M)$ перемещает точку $M$ вдоль плоской кривой $l$. При этом сила совершает работу по перемещению
    ...
    \vspace{3mm}
\textit{Nota}. Ориентированный участок поверхности $d\overrightarrow{\sigma}$ - это размер участка $d\sigma$, умноженный на вектор нормали к участку $\overrightarrow{n}$,
    ...
    1* Дробление области $\Omega$ на элементы $d\omega$
    2* Выбор постоянного значения функции на $d\omega$, то есть $g_{\text{ср.}}$ или $\overrightarrow{G_\text{ср.}}$
    3* Составление подынтегрального выражения $g_{\text{ср.}}d\omega$ или $\overrightarrow{G_\text{ср.}}d\overrightarrow{\omega}$
    4* \enquote{Суммирование} элементарных величин $\displaystyle \int gd\omega$ или $\displaystyle \int \overrightarrow{G}d\overrightarrow{\omega}$

    \vspace{10mm}

    \textbf{5.2. Классификация интегралов}

    1* По размерности $\Omega$


    $n = 1$: * прямая (определенный интеграл $\displaystyle \int_a^b$)

    * кривая (криволинейный интеграл $\displaystyle \int_A^B$)

    $n = 2$: * плоскость (двойной интеграл $\displaystyle \iint_D$)

    * поверхность, не криволинейная (поверхностный интеграл $\displaystyle \iint_S$)

    $n = 3$: * пространство $\Real^3$ (тройной $\displaystyle \iiint_V$ или $\displaystyle \iiint_T$)

    2* По виду функции

    скалярная $g(M)$ \quad векторная $\overrightarrow{G}(M)$

    $n = 1$ определенный, криволинейный I рода \quad криволин. II рода (интегралы в проекциях)

    $n = 2$ двойной, поверхн. I рода \quad поверхн. II рода
