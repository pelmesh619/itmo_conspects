...
\begin{document}
    \vspace{3mm}
\textit{Nota}. $Ker\ \mathcal{A}$ и $Im\ \mathcal{A}$ - подпространства $V$ ($\mathcal{A} : V \rightarrow V$)
    ...
    $\displaystyle \Sigma c_i x_i \subset \mathcal{A}$, если $\forall x_i \in Ker \mathcal{A}$
    ...
    \textbf{
    \vspace{3mm}
\textit{Nota}. Обратное также верно:}
...
\textit{Nota}. Также очевидно, что
    ...
    \vspace{3mm}
\textit{Th}. $\mathcal{A} : V \rightarrow V$, тогда $\dim Ker \mathcal{A} + \dim Im \mathcal{A} = \dim V$
    ...
    \vspace{3mm}
\textit{Def}. Рангом оператора $\mathcal{A}$ называется $\dim Im \mathcal{A}$: $rang \mathcal{A} \stackrel{def}{=} \dim Im \mathcal{A} (= r(\mathcal{A}) = rank \mathcal{A})$

    \vspace{3mm}
\textit{Nota}. Сравним ранг оператора с рангом его матрицы
    ...
    \vspace{3mm}
\textit{Nota}. Поиск матрицы $\mathcal{A}$ можно осуществить, найдя ее в \enquote{домашнем} базисе $\Set{e_i}$, то есть $A (e_1, \dots e_n) = (e_1^\prime, \dots, e_m^\prime)$
    ...
    \vspace{3mm}
\textit{Nota}. Рассмотрим преобразование координат, как линейный оператор $T : V^n \rightarrow V^n$ (переход из системы $Ox_i \rightarrow Ox_i^\prime$, $i = 1..n$)
    ...
    \vspace{3mm}
\textit{Th}. $\mathcal{A} : V^n \rightarrow V^n$
