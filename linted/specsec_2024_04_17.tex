...
\begin{document}
    \vspace{3mm}
\textit{Nota}. Особое значение имеют симметричные билинейные формы
    ...
    \vspace{3mm}
\textit{Def}. Квадратичной формой, порожденной Б. Ф. $\mathcal{B}(u, v)$, называется форма $\mathcal{B}(u, u)$

    \vspace{3mm}
\textit{Ex}. Поверхность
    ...
    \vspace{3mm}
\textit{Mem}. Ранее уравнение поверхности второго порядка (без линейной группы, то есть сдвига)

    \[a_{11}x^2 + 2a_{12}xy + a_{22}y^2 + 2a_{23}yz + 2a_{13}xz + a_{33}z^2 = c\]

    \vspace{3mm}
\textit{Nota}. Заметим, что здесь коэфф. $a_{ij}$ соответствуют матрице симметричной Б. Ф.:
    ...
    \vspace{3mm}
\textit{Def}. Положительно определенная форма

    \vspace{3mm}
\textit{Nota}. Можно говорить о положительно определенном операторе $\mathcal{A}: V^n \rightarrow V^n$
    ...
    \vspace{3mm}
\textit{Th}. 1), 2) \Longleftrightarrow $\ \forall \lambda_i$ - с. число $\mathcal{A}$, $\lambda_i > 0$
    ...
    \vspace{3mm}
\textit{Nota}. $\det A$ инвариантен при замене базиса, $\det A = \lambda_1 \cdot \dots \cdot \lambda_n > 0$. Тогда $\exists \mathcal{A}^{-1}$

    \vspace{3mm}
\textit{Th}. Критерий Сильвестра
    ...
    \vspace{3mm}
\textit{Ex}. Поверхность: $x^2 + y^2 + z^2 = 1$
    ...
    \vspace{3mm}
\textit{Def}. Оператор $\mathcal{A}$ называется отрицательно определенным, если $-\mathcal{A}$ - положительно определенный

    \vspace{3mm}
\textit{Nota}. Для $-\mathcal{A}$ работает критерий Сильвестра: $\Delta_k(-\mathcal{A}) =
    ...
    \vspace{3mm}
\textit{Nota}. Аналогично операторы определяются положительно или отрицательно билинейные формы
    ...
    \vspace{3mm}
\textit{Nota}. После приведения $\mathcal{B}(u, v)$ к каноническому виду, получаем
