...
* Isolation - изоляция: если запущено несколько конкурирующих транзакций, то любое обновление, выполненное одной
...
3.	Файловые хранилища. Недостатки. Упорядоченные и неупорядоченные файлы.
    ...
    [В файловом хранилище](#file_server_architecture) храним данные в одном файле:
...
4.	Трехуровневая архитектура ANSI/SPARC.
    ...
    [Трехуровневая архитектура (ANSI/SPARC)](#three_level_architecture):
        - Внешний 
        - Концептуальный
        - Внутренний (физический, исходный)
...
5.	Модель сущность-связь. Классификации сущностей, атрибутов и связей. Нотация Чена для представления модели сущность-связь.
    ...
    [Модель сущность-связь](#entity_relation_model):
...
6.	Логическая и физическая модели данных. Содержание уровней.
    ...
    [Уровни моделей данных](#data_model_levels):
        - Сущность-связь
        - Логический (дата-логический) уровень
        - Физический уровень
...
10.	Реляционная модель данных: терминология, свойства отношения.
    ...
    Введем [следующие термины](#relational_terms):
    ...
    [Свойства отношения](#relations_properties):
...
11.	Реляционная модель данных: виды ключей, реализация различных типов связей, виды целостности.
    ...
    [Определения ключей](#relations_keys):
    ...
    Тут же выделяем [два вида целостности](#integrity_types):
...
12.	Операции реляционной алгебры: проекция, выборка, объединение, разность, пересечение.
    ...
    [Операции](#relational_operations_1): 
    ...
    3. Объединение (Union)
        ...
        Синтаксически такие запросы в SQL разрешены, но семантически они не имеют смысла. 
...
13.	Операции реляционной алгебры: Тета-соединение, эквисоединение, естественное соединение, левое внешнее соединение, полусоединение.
    ...
    [Операции](#relational_operations_2): 
    ...
    6. Декартовое произведение
        ...
        Семантически декартовое произведение зачастую не имеет смысла. 
    ...
    9. Естественное соединение (Natural Join)
        ...
        Естественное соединение удобно, например, когда есть два таблицы с атрибутами серии и номера паспорта. 
...
14.	Структура и порядок выполнения предложения Select в SQL.
    ...
    Рассмотрим запрос [выборки в SQL](#select_instruction) - его можно разделить на 5 частей:
...
15.	Алгоритмы реализации соединений отношений в SQL.
    ...
    Рассмотрим [две реализации `JOIN`](#join_implementations):
