...
\begin{document}
    ...
    Def 1. Оператор $\mathcal{A}^*$ называется сопряженным для $\mathcal{A} : V \to V$, если

    $(\mathcal{A}x, y) = (x, \mathcal{A}^* y)$

    Def 2. $\mathcal{A}^*$ сопряженный для $\mathcal{A}$, если $A^* = A^T$ в любой ортонормированном базисе

    Nota. Def 1. \Longleftrightarrow Def \ 2.
    ...
    Ex 1. $A = \begin{pmatrix}1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1\end{pmatrix} = I$
    ...
    Ex 2. $A = \begin{pmatrix}0 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0\end{pmatrix} = \mathcal{O}$
    ...
    Ex 3. Поворот $\Real^2$ на $\frac{\pi}{4}$
